...
double Evaluator::NNrecall(unsigned *tpkList,unsigned row,unsigned dim,unsigned tpk,const char *grdDst)
{
    unsigned grow=0,gdim=0;
    unsigned irow,idim;
    unsigned i;
    unsigned matchCount=0;
    unsigned *grdtruth =IOAgent::loadgroundtruth(grdDst,grow,gdim);
    cout<<"gdim:"<<gdim<<endl;
    cout<<"tpk:"<<tpk<<endl;
    if(gdim<tpk)
    {
        cout<<"error!because gdim<topk"<<endl;
        return 0;
    }
    for(irow = 0;irow<row;irow++)
    {
        ...
        {
            unsigned tmpmat = tpkList[irow*dim+idim];

            for(i=0;i<tpk;i++)
            {
                 unsigned tmpgrd = grdtruth[irow*gdim+i];
                 if(tmpgrd==tmpmat)
                 {
                    matchCount++;
                 }
...
void Evaluator::test()
{
    ...
    */
    unsigned row,dim;
 //   const char *grdDst = "/home/pclin/nns_benchmark-master/data/glove1m/grd/glove1m_gold_knn40.txt";
 //   const char *srcfn = "/home/pclin/nns_benchmark-master/data/glove1m/result/glove1m_knngraph.txt";

   const char *grdDst = "/home/pclin/nns_benchmark-master/data/rand/grd/rand100k100d_gold_knn30.txt";
   const char *srcfn = "/home/pclin/nns_benchmark-master/data/rand/randNN/rand100k100dNN.txt";
  //const char *srcfn = "/home/pclin/nns_benchmark-master/data/glove1m/nn_result/nn_result.txt";
    unsigned *tpklist = IOAgent::loadgroundtruth(srcfn,row,dim);
  // unsigned *tpklist = IOAgent::loadKNNG(srcfn,row,dim);
    cout<<"tp row:"<<row<<endl;
    cout<<"tp dim:"<<dim<<endl;
    double r;
    r=Evaluator::NNrecall(tpklist,row,dim,30,grdDst);
    cout<<"recall="<<r<<endl;
}
