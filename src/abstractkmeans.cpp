...
bool AbstractKMeans::initMemry(const unsigned int dNum, const unsigned int clustNum)
{
    ...
    {    if(kmMtd == _rbkmn_ /**/ || kmMtd == _hek_/**/ || kmMtd == _hec_)
        {
...
{
    ///this function mainly to cal the distortion.
    double *dcts = new double[clustNum];
    double *C    = new double[this->ndim];
    double tmpE = 0, sumEg = 0, tmpEg = 0,sumDcts = 0;
...
double AbstractKMeans::normVects(float *vects, double *lens,
                                 const unsigned int dim, const unsigned int n0)
{
    assert(vects);
    assert(lens);
    unsigned int i = 0, j = 0, loc = 0;
