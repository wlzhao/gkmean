...
unsigned int *IOAgent::loadKNNG(const char *srcFn, unsigned int &row, unsigned int &dim)
{
    row = dim = 0;
    unsigned int *knnG = NULL, iRow = 0, i = 0, idx = 0, val = 0;
    ifstream *inStrm = new ifstream(srcFn);
    (*inStrm)>>row;
    (*inStrm)>>dim;
    knnG = new unsigned int[row*dim];
    while(!inStrm->eof() &&iRow < row)
    {
        for(i = 0; i < dim; i++, idx++)
        {
            (*inStrm)>>val;
...
unsigned int *IOAgent::loadKNNG1(const char *srcFn, unsigned int &row, unsigned int &dim)
{
    row = dim = 0;
    unsigned int *knnG = NULL, iRow = 0, i = 0, idx = 0, val = 0;
    unsigned int head = 0;
...
void IOAgent::mergKNNG(const char *knngFn1, const char *knngFn2, const char *dstKnng)
{
    unordered_map<unsigned int, set<unsigned int> * >::iterator mit;
    unordered_map<unsigned int, set<unsigned int> * > knnGMap;
    set<unsigned int>::iterator sit;
    set<unsigned int> *crntSet = NULL;
    unsigned int iRow = 0, i = 0, idx = 0, val = 0;
    unsigned int head = 0, row = 0, dim = 0;
    ifstream *inStrm = new ifstream(knngFn1);
    (*inStrm)>>row;
    (*inStrm)>>dim;
    for(i = 0; i < row; i++)
    {
          knnGMap.insert(pair<unsigned int, set<unsigned int> *> (i, new set<unsigned int>) );
    }
    while(!inStrm->eof() &&iRow < row)
    {
        (*inStrm)>>head;
        crntSet = knnGMap[head];
        for(i = 0; i < dim; i++, idx++)
        {
            (*inStrm)>>val;
            crntSet->insert(val);
    ...
    ofstream *outStrm = new ofstream(dstKnng, ios::out);
    (*outStrm)<<knnGMap.size()<<" "<<30<<endl;
    int minSz = 0;
    for(mit = knnGMap.begin(); mit != knnGMap.end(); mit++)
    {
         crntSet = mit->second;
         minSz   = crntSet->size()<30?crntSet->size():30;
         (*outStrm)<<mit->first;
         (*outStrm)<<" "<<minSz;
         for(sit = crntSet->begin(), idx = 0; sit != crntSet->end() && idx < 30; sit++, idx++)
         {
              (*outStrm)<<" "<<*sit;
         }
         (*outStrm)<<endl;
...
float *IOAgent::loadIMat(const char *srcfn, unsigned int &row, unsigned int &col)
{
    assert(srcfn);
    float vals[2] = {0};
    ...
    memset(mat, 0, sizeof(float)*row*col);
    float *tmp = new float[col];
    unsigned int irow, idim, loc, num = 0;
...
unsigned *IOAgent::loadgroundtruth(const char *gtdst,unsigned int &row,unsigned int &dim)
{
    unsigned irow,idim;
    unsigned id,tpk;
    unsigned tmp;
    ifstream *inS = new ifstream(gtdst,ios::in);
    if(!inS->is_open())
    {
        cout<<"File "<<gtdst<<" cannot open for read!\n";
        return NULL;
    }
    (*inS)>>row;
    (*inS)>>id;
    ...
    ifstream *inStrm = new ifstream(gtdst,ios::in);
    if(!inStrm->is_open())
    {
        cout<<"File "<<gtdst<<" cannot open for read!\n";
        return NULL;
...
void IOAgent::test()
{
    const char *path ="/home/pclin/nns_benchmark-master/data/glove1m/grd/glove1m_gold_knn40.txt";
    unsigned row,dim;
    IOAgent *io = new IOAgent();
    io->loadgroundtruth(path,row,dim);
}
